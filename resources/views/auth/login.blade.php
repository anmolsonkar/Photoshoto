        ...
        <div class="p-1.5 hidden md:block">
            ...
            <form>
                <div class="pt-2 relative mx-auto text-gray-600">
                    <x-jet-input class="border-2 border-gray-300 bg-white h-10 px-5 pr-16 rounded-lg text-sm hover:shadow"
            ...
            <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
                <div class="pt-2 pb-3 space-y-1">
                    @if (Route::has('login'))
                    @auth
                    <x-jet-responsive-nav-link class="text-indigo-600 visited:text-indigo-400 border-indigo-400 bg-gray-100" href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard')">
                        {{ __('Home') }}
                    </x-jet-responsive-nav-link>
                    @else
                    <x-jet-responsive-nav-link class="text-indigo-600 visited:text-indigo-400 border-indigo-400 bg-gray-100"  href="{{ route('login') }}" :active="request()->routeIs('login')">
                        {{ __('Login') }}
                    </x-jet-responsive-nav-link>
                    @if (Route::has('register'))
                    <x-jet-responsive-nav-link class="text-indigo-600 visited:text-indigo-400 border-indigo-400 bg-gray-100"  href="{{ route('register') }}" :active="request()->routeIs('register')">
                        {{ __('Register') }}
                    </x-jet-responsive-nav-link>
                    @endif
                    @endauth
    ...
    <x-guest-layout>
        <x-jet-authentication-card>
            <x-slot name="logo">
                ...
                <img src="storage/svg/LOGO.png" class="w-16 lg:w-28 mt-5 lg:mt-0 md:mt-0" alt="Logo">
            </x-slot>
