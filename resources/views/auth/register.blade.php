...
<body class=" antialiased bg-gray-100">
    <div class="fixed w-full flex bg-white ">
        ...
        <nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
                <div class="pt-2 pb-3 space-y-1">
                    @if (Route::has('login'))
                    @auth
                    <x-jet-responsive-nav-link class="text-indigo-600 visited:text-indigo-400  border-indigo-400 bg-gray-100"
        ...
        <div class="hidden top-0 right-0 px-6 py-4 sm:block">
            @auth
            <a h </div>ref="{{ url('/dashboard') }}" class="text-sm text-gray-700 no-underline">
                <x-jet-button>
                    {{ __('Home') }}
                </x-jet-button>
            </a>
    ...
    <x-guest-layout>
        <x-jet-authentication-card>
            <x-slot name="logo">
                ...
                <img src="storage/svg/LOGO.png" class="w-16 lg:w-28 mt-5 lg:mt-16 md:mt-16" alt="Logo">
            </x-slot>
